	...
	merchants: [
		{
			...
			isOpen: 'Y',
			highlightText:
				'<strong>ร้านทุกอย่าง</strong><img src="javascript:alert(\'YOU HAVE BEEN HACKED\')" onError="javascript:alert(\'YOU HAVE BEEN HACKED\')" /> โต๊ะ ตู้ เตียง',
...
export const useApi = () => {
	...
	const fetchData = async () => {
		try {
			...
			// [!] Proxy Server is no longer available.
			// console.warn(
			...
			// }
			console.warn('[!] An error occurred when fetching from direct origin. Serving saved data.')
			setPartialData({ data: ywc18data, state: API_STATES.SUCCESS })
		}
	}
